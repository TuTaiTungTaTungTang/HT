...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $img_file = empty($errors) ? handle_image_upload() : false;
                ...
                <form method="POST" enctype="multipart/form-data" class="col-md-6 offset-md-3">
                    ...
                    <div class="form-group">
                        ...
                        <div class="d-flex gap-3 flex-wrap">
                            <?php foreach (['XS', 'M', 'L', 'Freezie'] as $sz) : ?>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="pd_sizes[]" value="<?= $sz ?>" id="sz_<?= $sz ?>" <?= (isset($_POST['pd_sizes']) && in_array($sz, (array)$_POST['pd_sizes'])) ? 'checked' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="pd_collection" id="pd_collection" class="form-select fs-5">
                            <?php foreach ($bstCollectionOptions as $collectionCode => $collectionLabel) : ?>
                                <option value="<?= html_escape($collectionCode) ?>" <?= (($_POST['pd_collection'] ?? '') === $collectionCode) ? 'selected' : '' ?>>
